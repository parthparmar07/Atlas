...
type ExamAction = "Schedule Exams" | "Check for Clashes" | "Optimize Schedule";
...
interface ExamOpsControlProps {
  schoolId: string;
  defaultCourseScope?: string[];
}

const ACTIONS: RunnerAction[] = [
  {
    label: "Schedule Exams",
    description: "Generate a clash-safe baseline schedule with hall assignment.",
  },
  {
    label: "Check for Clashes",
    description: "Audit conflicts across section, hall, and capacity constraints.",
  },
  {
    label: "Optimize Schedule",
    description: "Reduce load spikes and optimize room utilization.",
  },
];

const DEFAULT_HALLS = "Hall A|120\nHall B|90\nLab 1|60";
...
export default function ExamOpsControl({ schoolId, defaultCourseScope = [] }: ExamOpsControlProps) {
  const [examCycle, setExamCycle] = useState("End Semester");
  const [courseScopeInput, setCourseScopeInput] = useState(
    defaultCourseScope.length > 0 ? defaultCourseScope.join("\n") : "B.Tech CSE Semester 4\nB.Tech CSE Semester 6"
  );
  const [hallsInput, setHallsInput] = useState(DEFAULT_HALLS);

  const [runningAction, setRunningAction] = useState<ExamAction | null>(null);
  const [activeAction, setActiveAction] = useState<ExamAction | null>(null);
  const [response, setResponse] = useState<RunResponse | null>(null);
  const [error, setError] = useState("");

  const backendBase = useMemo(() => process.env.NEXT_PUBLIC_API_URL ?? "http://localhost:8000", []);

  const parseCourseScope = () => {
    return courseScopeInput
      .split("\n")
      .map((line) => line.trim())
      .filter(Boolean);
  };
  ...
  const runAction = async (action: ExamAction) => {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : "Failed to execute exam operation.");
    } finally {
  ...
  const normalizedOutput = useMemo(() => {
    ...
    if (activeAction === "Check for Clashes") {
      const clashes = (byStatus.get("exam_clash_report")?.clashes as Record<string, unknown>[]) || [];
      return {
        kpis: [
          { label: "Total Clashes", value: Number(byStatus.get("exam_clash_report")?.clash_count || clashes.length) },
          {
            label: "Critical",
            value: clashes.filter((row) => String(row?.severity || "").toLowerCase() === "critical").length,
          },
          {
            label: "Capacity Overflow",
            value: clashes.filter((row) => String(row?.type || "") === "capacity_overflow").length,
          },
        ],
        table: {
    ...
    const adjustments = (byStatus.get("exam_optimization_completed")?.adjustments as Record<string, unknown>[]) || [];
    return {
      kpis: [
        { label: "Total Adjustments", value: adjustments.length },
        {
          label: "Rescheduled",
          value: adjustments.filter((row) => row?.old_day && row?.new_day).length,
        },
        {
          label: "Hall Changes",
          value: adjustments.filter((row) => row?.old_hall && row?.new_hall).length,
        },
      ],
      table: {
    ...
    <div className="bg-white rounded-3xl border border-slate-100 p-6 shadow-xl space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <h3 className="text-xl font-black text-slate-900">Exam Ops Control</h3>
          <p className="text-xs text-slate-500 font-medium mt-1">Proper inputs in, structured schedule output out.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        ...
        <label className="text-xs font-bold text-slate-500 md:col-span-2">
          Course Scope (one per line)
          <textarea
            value={courseScopeInput}
            onChange={(e) => setCourseScopeInput(e.target.value)}
            rows={3}
      ...
      <label className="text-xs font-bold text-slate-500 block">
        Hall Capacity Map (Hall|Capacity per line)
        <textarea
          value={hallsInput}
          onChange={(e) => setHallsInput(e.target.value)}
          rows={3}
          className="mt-1 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-700"
        />
      </label>
      ...
      {response ? (
        <div className="rounded-3xl border border-slate-200 bg-slate-50 p-5 space-y-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <div className="text-sm font-black text-slate-900">{response.result?.title || "Exam operation completed"}</div>
                      ...
                      <tr key={index} className="border-b last:border-b-0 border-slate-100">
                        {(normalizedOutput?.table?.columns || []).map((column) => (
                          <td key={`${index}-${column}`} className="px-4 py-2.5 text-xs text-slate-700 align-top">
                            <span className="block max-w-[280px] truncate" title={formatCell(row?.[column])}>
