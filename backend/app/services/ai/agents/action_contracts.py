...
AGENT_ACTION_CONTRACTS: Dict[str, Dict[str, Dict[str, Any]]] = {
    ...
    "admissions-leads": {
        "Send Campaigns": {"handler": "admissions_followup", "required_inputs": ["segments[]", "programme", "channel"]},
        "Check Drop-offs": {"handler": "admissions_funnel", "required_inputs": ["leads[].engagement", "leads[].last_activity_at"]},
        "Segment Leads": {"handler": "admissions_qualify", "required_inputs": ["leads[].name", "leads[].programme_interest", "leads[].source"]},
        "Match Scholarships": {"handler": "admissions_scholarship", "required_inputs": ["lead.score", "lead.category(optional)", "lead.annual_income(optional)"]},
    },
    "admissions-documents": {
        "Verify Batch": {"handler": "admissions_parse_documents", "required_inputs": ["documents[]"]},
        "Flag Issues": {"handler": "admissions_parse_documents", "required_inputs": ["documents[]"]},
        "Generate Checklist": {"handler": "admissions_brief", "required_inputs": ["programme_level", "documents_required[]"]},
        "Push to ERP": {"handler": "admissions_brief", "required_inputs": ["verified_records[]"]},
    },
    ...
    "hr-bot": {
        "Process Leaves": {"handler": "hr_leave", "required_inputs": ["leave_requests[]"]},
        "Draft Notice": {"handler": "hr_policy", "required_inputs": ["notice_topic", "audience"]},
        "Payroll Summary": {"handler": "hr_appraisal", "required_inputs": ["payroll[]"]},
        "Onboarding Checklist": {"handler": "hr_onboarding", "required_inputs": ["employees[]"]},
    },
    "hr-load-balancer": {
    ...
    "academics-calendar": {
        "Generate Calendar": {"handler": "academics_calendar", "required_inputs": ["year", "holidays[]", "exam_windows[]"]},
        "Add Event": {"handler": "academics_calendar", "required_inputs": ["event.name", "event.date", "event.type"]},
        "Holiday Mapping": {"handler": "academics_calendar", "required_inputs": ["holidays[]", "teaching_days[]"]},
        "Export Calendar": {"handler": "academics_calendar", "required_inputs": ["calendar[]"]},
    },
    ...
    "placement-resume": {
        "Score Resumes": {"handler": "placement_resume", "required_inputs": ["resumes[]", "target_jd"]},
        "Optimise Resume": {"handler": "placement_resume", "required_inputs": ["resume_text", "target_role"]},
        "Bulk Audit": {"handler": "placement_resume", "required_inputs": ["resumes[]"]},
        "JD Matcher": {"handler": "placement_jd", "required_inputs": ["jd_text"]},
    },
    ...
    "academics-curriculum": {
        "Audit Syllabus": {"handler": "academics_curriculum", "required_inputs": ["programme", "syllabus[]", "benchmarks[]"]},
        "NEP Compliance": {"handler": "academics_curriculum", "required_inputs": ["programme", "credits[]", "obe_docs[]"]},
        "Industry Alignment": {"handler": "academics_curriculum", "required_inputs": ["track", "industry_roles[]"]},
        "Generate Audit Report": {"handler": "academics_curriculum", "required_inputs": ["programmes[]"]},
    },
    "students-wellbeing": {
